    ...
    let hostSource = 'generic'; // character pool for the next room: 'generic' | 'favorites' | 'mix' (Detective Showdown games)
    let hostPool = 'random';    // 🎲 Hot & Cold + Blur Guess pool: 'random' (full website pool) | 'watched' (only anime the synced accounts have seen)
    let hostMixCount = 12;      // 🔀 Detective Showdown Mix split: how many board slots come from the generic pool
    ...
    async function createGameRoom() {
      ...
      try {
        ...
        document.getElementById('lobbyGameName').innerHTML = ic(GAME_ICONS[game] || 'gamepad') + ' ' + (GAME_LABELS[game] || 'Detective Showdown');
    ...
    function afterJoinUI(room) {
      showScreen('lobbyScreen');
      document.getElementById('displayRoomCode').textContent = roomCode;
      document.getElementById('lobbyGameName').innerHTML = ic(GAME_ICONS[room.game] || 'gamepad') + ' ' + (GAME_LABELS[room.game] || 'Detective Showdown');
    ...
    function updateLobby() {
      ...
      if (gameNameEl) gameNameEl.innerHTML = ic(GAME_ICONS[currentRoom.game] || 'gamepad') + ' ' + (GAME_LABELS[currentRoom.game] || 'Detective Showdown');
    ...
    function renderBattleBoard() {
      ...
      (currentRoom.characters || []).forEach(char => {
        ...
        // with a big ✕ as in the 1v1 Detective Showdown. Tap the card again to undo.
    ...
    function renderMultiEnd(kind) {
      ...
      if (kind === 'battle') {
        ...
        const myTeam = cnTeamOf(playerId, cn);
        title.innerHTML = winTeam ? cnEmoji(winTeam) + ' ' + escapeHtml((winTeam === 'red' ? (window.t ? t('RED') : 'RED') : (window.t ? t('BLUE') : 'BLUE'))) + ' ' + (window.t ? t('team wins!') : 'team wins!') : (window.t ? t('Ninja Scrolls over') : 'Ninja Scrolls over');
    ...
    // 🔀 Mix split bar (Detective Showdown games, pool = Mix) — reads/writes settings/mixCount
    function syncMixUI() {
      const g = (currentRoom && currentRoom.game) || 'guesswho';
